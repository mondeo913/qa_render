...
class QaUniverseSeeder extends Seeder
{
    ...
    {
        $this->call([RolePermissionSeeder::class, AgencyTemplateSeeder::class]);
        ...
        DB::transaction(function (): void {
            $agencies = ContractingAgency::query()
                ...
                ->orderBy('id')->get();
            if ($agencies->count() !== 2) throw new \RuntimeException('El universo QA requiere las dependencias IMSS e IPAB.');
    ...
    {
        $qaLoads = ScheduledLoad::query()->whereRaw("COALESCE(metadata->>'qa','false') = 'true'")->pluck('id');
        if ($qaLoads->isNotEmpty()) ScheduledLoad::query()->whereIn('id', $qaLoads)->delete();
        CalendarImport::query()->where('original_filename', 'like', 'Pauta_QA_%')->delete();
        UserScope::query()->whereHas('user', fn ($query) => $query->whereRaw("COALESCE(metadata->>'qa','false') = 'true'"))->delete();
        $canonicalEmails = ['admin@siget.local','director.general@siget.local','director@siget.local','director.produccion@siget.local','enlace@siget.local','operador.monitoreo@siget.local','operacion.programacion@siget.local','fiscalizador@siget.local','director.monitoreo.IPAB@siget.local','director.produccion.IPAB@siget.local','enlace.IPAB@siget.local','operador.monitoreo.IPAB@siget.local','operacion.programacion.IPAB@siget.local','fiscalizador.IPAB@siget.local'];
        $duplicateUsers = User::query()->whereRaw("COALESCE(metadata->>'qa','false') = 'true'")->whereNotIn('email', $canonicalEmails)->get();
        if ($duplicateUsers->isEmpty()) return;
        $canonicalByRole = User::query()->whereIn('email', $canonicalEmails)->with('role')->get()->keyBy(fn (User $user) => $user->role?->code ?? 'ADMINISTRADOR');
        foreach ($duplicateUsers as $user) {
            ...
            $references = DB::select(<<<'SQL'
                ...
                WHERE tc.constraint_type = 'FOREIGN KEY' AND ccu.table_name = 'users' AND ccu.column_name = 'id' AND tc.table_name NOT IN ('users','user_scopes')
            SQL);
            foreach ($references as $reference) DB::table($reference->table_name)->where($reference->column_name, $user->id)->update([$reference->column_name => $replacement->id]);
            $user->delete();
        }
    }
    ...
    private function createQaUsers($agencies): array
    {
        ...
        $users = [];
        $globalDefinitions = [['ADMINISTRADOR','admin@siget.local','Administrador QA',null],['DIRECTOR_GENERAL','director.general@siget.local','Director General QA',null]];
        foreach ($globalDefinitions as [$roleCode,$email,$name,$unit]) $users[$email] = $this->upsertQaUser($roleCode,$email,$name,$unit?->id,$password);
        foreach ($agencies as $agency) {
            ...
            foreach ($definitions as [$roleCode,$email,$name,$unit]) $users[$email] = $this->upsertQaUser($roleCode,$email,$name,$unit?->id,$password);
        }
        return $users;
    }
    ...
    private function upsertQaUser(string $roleCode,string $email,string $name,?int $unitId,string $password): User
    {
        ...
        $user = User::query()->updateOrCreate(['email'=>$email],[
            'role_id'=>$role->id,'contracting_agency_id'=>null,'organizational_unit_id'=>$unitId,'name'=>$name,'password'=>$password,'status'=>'ACTIVE','email_verified_at'=>now(),'metadata'=>['qa'=>true,'source'=>'QaUniverseSeeder'],
        ]);
    ...
    private function seedPautasThroughRealFlow($agencies,array $users): void
    {
        ...
        foreach ($agencies as $agency) {
            ...
            try {
                ...
                $upload = new UploadedFile($tempPath,"Pauta_QA_{$agency->code}_2026.xlsx",'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',UPLOAD_ERR_OK,true);
    ...
    private function buildPautaWorkbook(string $agencyCode,int $year): Spreadsheet
    {
        ...
        $rows = [3=>['Campaña institucional '.$agencyCode,'v1','Nacional','Canal Catorce','Nacional','SIGET','Canal Catorce','Barra institucional','Nacional','','','Contenido general','08:00-23:59','SPOT / 30 seg'],4=>['Campaña institucional '.$agencyCode,'v1','Nacional','Canal Catorce','Nacional','SIGET','Canal Catorce','Continuidad','Nacional','','','Promoción institucional','08:00-23:59','SPOT / 30 seg']];
        foreach ($rows as $rowNumber=>$values) foreach ($values as $index=>$value) $sheet->setCellValue(Coordinate::stringFromColumnIndex($index+1).$rowNumber,$value);
        $cursor=15;
        foreach (array_keys($months) as $monthNumber) {
            $daysInMonth = CarbonImmutable::create($year,$monthNumber,1)->daysInMonth;
            foreach (range(1,$daysInMonth) as $day) {
                if (in_array($day,$days,true)) {
                    $letter = Coordinate::stringFromColumnIndex($cursor);
                    $sheet->setCellValue($letter.'3','X');
                    if ($day===10 || $day===24 || $day===28) $sheet->setCellValue($letter.'4','X');
                }
                $cursor++;
            }
        }
