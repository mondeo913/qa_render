...
$dl = [];
$dv = [];
foreach ($directionRows as $r) {
    $dl[] = $r['unit'] ?? 'Sin unidad';
    $dv[] = (float) ($r['percentage'] ?? 0);
}
$ml = [];
$mc = [];
foreach ($monthlyRows as $r) {
    $ml[] = $r['period'] ?? '';
    $mc[] = (float) ($r['compliance'] ?? 0);
}
$builderRows = $loads->map(function ($l) {
    $unit = $l->deliverables
        ...
        ->implode(' / ');
    return [
        'agency' => $l->agency?->name ?: 'Sin dependencia',
        'unit' => $unit ?: '—',
        'title' => $l->title,
        'status' => $l->status instanceof \BackedEnum ? $l->status->value : (string) $l->status,
        'progress' => (float) $l->completion_percentage,
        'open' => $l->effective_open_at?->format('d/m/Y H:i') ?: '—',
        'close' => $l->effective_close_at?->format('d/m/Y H:i') ?: '—',
        'evidence' => $l->deliverables->flatMap->evidences->count(),
    ];
})->values();
@endphp
...
.crtop{padding:18px 20px;margin-bottom:15px;border-radius:12px;background:linear-gradient(135deg,var(--side),#203c63);color:#fff}
.crtop h2{margin:0;color:#fff}.crtop p{margin:3px 0 0;color:#d4dfec;font-size:.75rem}
.toolbar{padding:14px;margin-bottom:15px}.tabs{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:15px}
.tab{padding:8px 11px;border:1px solid var(--border);border-radius:9px;background:var(--surface);color:var(--text);font-weight:700;font-size:.7rem;cursor:pointer}
.tab.on{background:var(--primary);color:#fff}.panel{display:none}.panel.on{display:block}
.kpis{display:grid;grid-template-columns:repeat(6,1fr);gap:9px;margin-bottom:15px}.crk{padding:11px}.crk small{display:block;color:var(--muted);font-size:.6rem;text-transform:uppercase}.crk strong{font-size:1.3rem}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:15px}.head{padding:12px 14px;background:var(--surface2);border-bottom:1px solid var(--border)}.head h3{margin:0;font-size:.9rem;color:var(--text)}
.chart{height:280px;padding:10px}.band{padding:8px 11px;background:var(--side);color:#fff;font-weight:800;font-size:.7rem}
.tbl{width:100%;border-collapse:collapse;font-size:.68rem;color:var(--text)}.tbl th{background:var(--side);color:#fff;padding:7px;text-align:left;font-size:.58rem}.tbl td{padding:7px;border-bottom:1px solid var(--border)}.tbl tr:nth-child(even) td{background:var(--surface2)}
.builder-grid{display:grid;grid-template-columns:1.05fr .95fr;gap:15px;padding:15px}.builder-field{padding:12px;border:1px solid var(--border);border-radius:10px;background:var(--surface2);margin-bottom:10px}.builder-field label{display:block;font-weight:800;font-size:.68rem;margin-bottom:5px}
.builder-checks{display:grid;grid-template-columns:1fr 1fr;gap:7px}.builder-check{display:flex;gap:7px;align-items:center;padding:7px 8px;border:1px solid var(--border);border-radius:8px;background:var(--surface)}.builder-check label{margin:0;font-weight:600;font-size:.65rem}
.builder-preview{min-height:260px;border:1px dashed var(--border);border-radius:10px;background:var(--surface2);padding:12px}.builder-actions{display:flex;gap:8px;flex-wrap:wrap}.builder-help{font-size:.66rem;color:var(--muted);margin-top:4px}
@media(max-width:1000px){.kpis{grid-template-columns:repeat(3,1fr)}.grid,.builder-grid{grid-template-columns:1fr}}@media(max-width:600px){.kpis{grid-template-columns:repeat(2,1fr)}.builder-checks{grid-template-columns:1fr}}
...
<div class="siget-report-shell">
    <div class="crtop">
        <div style="font-size:.62rem;text-transform:uppercase;letter-spacing:.08em;opacity:.75">SIGET · Centro de Reportes · Crystal</div>
        <h2>Reportes Ejecutivos y de Auditoría</h2>
        <p>Dependencia → Dirección / Unidad → Orden / Pauta.</p>
    </div>
    ...
    <section id="exec" class="panel on">
        <div class="kpis">
            <div class="crk"><small>Total</small><strong>{{ number_format((int)($k['total'] ?? 0)) }}</strong></div>
            <div class="crk"><small>Activas</small><strong>{{ number_format((int)($k['active'] ?? 0)) }}</strong></div>
            <div class="crk"><small>Cerradas</small><strong>{{ number_format((int)($k['closed'] ?? 0)) }}</strong></div>
            <div class="crk"><small>Vencidas</small><strong>{{ number_format((int)($k['overdue'] ?? 0)) }}</strong></div>
            <div class="crk"><small>Reprogramadas</small><strong>{{ number_format((int)($k['reprogrammed'] ?? 0)) }}</strong></div>
            <div class="crk"><small>Cumplimiento</small><strong>{{ number_format((float)($k['compliance'] ?? 0),1) }}%</strong></div>
        </div>
        <div class="grid">
            <div class="crbox"><div class="head"><h3>Avance por Dirección / Unidad</h3></div><div class="chart"><canvas id="sigetReportDirection"></canvas></div></div>
            <div class="crbox"><div class="head"><h3>Tendencia de cumplimiento mensual</h3></div><div class="chart"><canvas id="sigetReportTrend"></canvas></div></div>
        </div>
        <div class="crbox" style="margin-top:15px">
            <div class="head"><h3>Resumen por dependencia</h3></div>
            @foreach($loadsByAgency as $agency=>$items)
                <div class="band">{{ $agency }} · {{ $items->count() }} cargas</div>
                <table class="tbl"><thead><tr><th>Orden</th><th>Pauta</th><th>Estado</th><th>Avance</th></tr></thead><tbody>
                @foreach($items as $load)
                    @php $loadStatus = $load->status instanceof \BackedEnum ? $load->status->value : (string) $load->status; @endphp
                    <tr><td>{{ $load->title }}</td><td>{{ $load->period_label ?: '—' }}</td><td>{{ $statuses[$loadStatus] ?? $loadStatus }}</td><td>{{ number_format((float)$load->completion_percentage,0) }}%</td></tr>
                @endforeach
                </tbody></table>
            @endforeach
        </div>
    ...
    <section id="comp" class="panel"><div class="crbox"><div class="head"><h3>Cumplimiento y Desempeño</h3></div><table class="tbl"><thead><tr><th>Dependencia</th><th>Programado</th><th>Reprogramado</th><th>Validado y cerrado</th><th>Vencido</th></tr></thead><tbody>
        @foreach($agencies as $a)
            @php $agencyLoads = $loads->filter(fn($l) => $l->agency?->id === $a->id); @endphp
            <tr><td>{{ $a->name }}</td><td>{{ $agencyLoads->where('status','PROGRAMADA')->count() }}</td><td>{{ $agencyLoads->where('status','REPROGRAMADA')->count() }}</td><td>{{ $agencyLoads->where('status','VALIDADO_Y_CERRADO')->count() }}</td><td>{{ $agencyLoads->where('status','VENCIDA')->count() }}</td></tr>
        @endforeach
    </tbody></table></div></section>

    <section id="deps" class="panel"><div class="crbox"><div class="head"><h3>Dependencias</h3></div><div class="chart"><canvas id="sigetReportUnits"></canvas></div><table class="tbl"><thead><tr><th>Dependencia</th><th>Cargas</th><th>Cerradas</th><th>Vencidas</th><th>Reprogramadas</th><th>Cumplimiento</th></tr></thead><tbody>
        @foreach($agencyRows as $r)<tr><td>{{ $r['agency'] ?? 'Sin dependencia' }}</td><td>{{ $r['total'] ?? 0 }}</td><td>{{ $r['closed'] ?? 0 }}</td><td>{{ $r['overdue'] ?? 0 }}</td><td>{{ $r['reprogrammed'] ?? 0 }}</td><td>{{ $r['percentage'] ?? 0 }}%</td></tr>@endforeach
    </tbody></table></div></section>
...
document.querySelectorAll('[data-r]').forEach(function(btn){
    btn.addEventListener('click',function(){
        ...
        var target=document.getElementById(btn.dataset.r);
...
(function(){
    ...
    function render(){
        ...
        var grouped={};
        rows.forEach(function(r){var key=r[group] ?? '—';if(!grouped[key]) grouped[key]=[];grouped[key].push(r);});
        ...
        Object.keys(grouped).sort().forEach(function(key){
            ...
            html+='</tr></thead><tbody>';
            grouped[key].forEach(function(r){html+='<tr>';cols.forEach(function(c){var v=r[c[0]];if(c[0]==='status')v=labels[v] ?? v;if(c[0]==='progress')v=Number(v).toFixed(0)+'%';html+='<td>'+esc(v)+'</td>';});html+='</tr>';});
...
<script type="application/json" data-siget-chart="sigetReportDirection">{!! json_encode(['type'=>'bar','data'=>['labels'=>$dl,'datasets'=>[['label'=>'Avance realizado %','data'=>$dv]]]]) !!}</script>
<script type="application/json" data-siget-chart="sigetReportTrend">{!! json_encode(['type'=>'line','data'=>['labels'=>$ml,'datasets'=>[['label'=>'Cumplimiento %','data'=>$mc,'fill'=>false]]]]) !!}</script>
<script type="application/json" data-siget-chart="sigetReportUnits">{!! json_encode(['type'=>'bar','data'=>['labels'=>$agencyRows->map(fn($r)=>$r['agency'] ?? 'Sin dependencia')->values(),'datasets'=>[['label'=>'Cumplimiento por dependencia %','data'=>$agencyRows->map(fn($r)=>(float)($r['percentage'] ?? 0))->values()]]]]) !!}</script>
@endsection