...
@section('content')
<style>
    /* Mis cargas: layout amplio, legible y sin alterar la lógica del repositorio operativo. */
    .mis-cargas-page {
        width: 100%;
        max-width: none;
        margin: 0;
    ...
    .mis-cargas-page .siget-card {
        width: 100%;
        max-width: none;
        margin: 0;
    }

    .mis-cargas-page .siget-file-toolbar {
        padding: 1.25rem 1.5rem;
        border-bottom: 1px solid var(--bs-border-color);
    }

    .mis-cargas-page .siget-file-section {
        padding: 1.5rem;
    }

    .mis-cargas-page .filter-card {
        position: sticky;
        top: .75rem;
        z-index: 5;
    ...
    .mis-cargas-page .dependency-card {
        overflow: hidden;
    }

    .mis-cargas-page .dependency-header {
        padding: 1rem 1.25rem;
    }

    .mis-cargas-page .loads-grid {
        display: grid;
        grid-template-columns: repeat(2, minmax(0, 1fr));
        gap: 1rem;
    }

    .mis-cargas-page .load-card {
        height: 100%;
        margin: 0 !important;
        display: flex;
    ...
    .mis-cargas-page .load-card > .card-body {
        flex: 1;
    }

    .mis-cargas-page .deliverables-grid {
        display: grid;
        grid-template-columns: repeat(2, minmax(0, 1fr));
        gap: 1rem;
    }

    .mis-cargas-page .deliverable-card {
        min-width: 0;
        height: 100%;
    }

    .mis-cargas-page .deliverable-title {
        overflow-wrap: anywhere;
    }

    .mis-cargas-page .evidence-actions {
        display: flex;
        flex-wrap: wrap;
        gap: .5rem;
        align-items: center;
    ...
    .mis-cargas-page .evidence-actions form {
        margin: 0;
    }
    ...
    <section class="card siget-card">
        <div class="siget-file-toolbar d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                ...
                <strong class="fs-5">Captura y cierre de evidencias programadas</strong>
            </div>
        ...
        <div class="siget-file-section">
            ...
            <form method="GET" action="{{ route('loads.mine') }}" class="card border mb-4 filter-card">
                <div class="card-header bg-body py-3">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <strong>Filtrar evidencias programadas</strong>
                    </div>
                </div>
                <div class="card-body">
            ...
            @forelse($loads->groupBy(function ($load) {
                ...
                <section class="card border mb-4 dependency-card">
                    <div class="card-header bg-body dependency-header d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            <div class="small text-secondary text-uppercase">Dependencia</div>
                            <h2 class="h4 mb-0">{{ optional($agency)->name ?: 'Sin dependencia' }}</h2>
                        </div>
                        <span class="badge text-bg-primary fs-6 px-3 py-2">{{ $agencyLoads->count() }} carga(s) programada(s)</span>
                    </div>
                        ...
                        <div class="loads-grid">
                            @foreach($agencyLoads as $load)
                                <article class="card border load-card">
                                    <div class="card-header bg-body d-flex justify-content-between align-items-start gap-3 flex-wrap p-3">
                                        <div class="min-w-0">
                                            <div class="small text-secondary">
                                                {{ $load->period_label }} · {{ optional($load->effective_open_at)->format('d/m/Y H:i') }}
                                            </div>
                                            <h3 class="h5 mb-1 mt-1">{{ $load->title }}</h3>
                                            <div class="small text-secondary"><i class="bi bi-file-earmark-text me-1"></i>{{ optional($load->template)->name ?: 'Pauta contratada' }}</div>
                                        </div>
                                        <a href="{{ route('loads.show', $load) }}" class="btn btn-sm btn-outline-primary flex-shrink-0"><i class="bi bi-folder2-open me-1"></i>Ver carga</a>
                                    </div>
                                    ...
                                    <div class="card-body p-3">
                                        <div class="deliverables-grid">
                                            @forelse($load->deliverables as $deliverable)
                                                ...
                                                @endphp
                                                <div class="border rounded-3 p-3 deliverable-card d-flex flex-column">
                                                    <div class="d-flex justify-content-between gap-2 flex-wrap">
                                                        <span class="badge text-bg-light"><i class="bi bi-diagram-3 me-1"></i>{{ optional($direction)->name ?: 'Sin dirección' }}</span>
                                                        @if($closed)
                                                            <span class="badge text-bg-success"><i class="bi bi-check-circle me-1"></i>Cerrada</span>
                                                        @elseif($evidence)
                                                    ...
                                                    <h4 class="h6 mt-3 mb-1 deliverable-title">{{ optional($requirement)->name ?: 'Entregable programado' }}</h4>
                                                    ...
                                                    <div class="mt-auto">
                                                        @if(!$evidence)
                                                            <form action="{{ route('evidences.store') }}" method="POST" enctype="multipart/form-data">
                                                                ...
                                                                <div class="input-group">
                                                                    ...
                                                                    <button class="btn btn-primary" type="submit"><i class="bi bi-paperclip me-1"></i>Adjuntar</button>
                                                                </div>
                                                            </form>
                                                        @elseif(!$closed)
                                                            <div class="evidence-actions">
                                                                ...
                                                                <form action="{{ route('evidences.submit', $evidence) }}" method="POST">
                                                                    @csrf
                                                                    <button class="btn btn-sm btn-success" type="submit" @if($currentFiles < $minFiles) disabled @endif><i class="bi bi-send-check me-1"></i>Cerrar y enviar</button>
                                                                </form>
                                                            </div>
                                                        ...
                                                        @else
                                                            <div class="evidence-actions">
                                                                <a href="{{ route('evidences.show', $evidence) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-eye me-1"></i>Consultar</a>
                                                                <span class="small text-success"><i class="bi bi-check2-circle me-1"></i>Disponible para revisión institucional y por dirección.</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
            ...
            @empty
                <div class="text-center py-5 text-secondary"><i class="bi bi-inbox fs-1 d-block mb-3"></i><h3 class="h5">No hay evidencias programadas</h3><p class="mb-0">No tienes evidencias asignadas dentro de tu alcance con los filtros seleccionados.</p></div>
            @endforelse
