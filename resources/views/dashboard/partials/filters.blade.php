...
<div class="col-xl-3 col-md-6"><label class="form-label">Campaña o pauta</label><select name="campaign" class="form-select"><option value="">Todas las pautas visibles</option>@foreach(($filterCampaigns ?? []) as $campaign)<option value="{{ $campaign }}" @selected(($filters['campaign'] ?? '') === $campaign)>{{ $campaign }}</option>@endforeach</select></div>
