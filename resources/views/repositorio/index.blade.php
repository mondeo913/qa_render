...
<div class="siget-file-manager" data-file-manager>
    ...
    <section class="card siget-card">
        ...
        <div class="siget-file-section border-top">
            ...
            @php($selectedSummary = $dependencySummaries->firstWhere('agency.id',(int)$filters['agency_id']))
            @if($selectedSummary)
                @forelse($selectedSummary->monthlyPautas as $group)
                    @php
                        ...
                        $statuses = $group->loads->map(fn($l) => $l->status instanceof \BackedEnum ? $l->status->value : $l->status)->unique()->join(', ');
                    @endphp
                    <div class="card border mb-4">
                        <div class="card-header bg-body d-flex justify-content-between align-items-center gap-3 flex-wrap">
                            <div>
                                ...
                                <h3 class="h5 mb-1"><i class="bi bi-folder2-open me-1"></i>{{ $group->template?->name ?? $firstLoad?->title ?? 'Pauta contratada' }}</h3>
                            ...
                            <div class="d-flex gap-2 flex-wrap">
                                ...
                                @endif
                                @if($from)
                                    <a href="{{ route('reports.pdf',['agency_id'=>$selectedSummary->agency->id,'from'=>$from->format('Y-m-d'),'to'=>($to?->format('Y-m-d') ?? $from->format('Y-m-d'))]) }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-file-earmark-pdf me-1"></i>Reporte general</a>
                                @endif
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table class="table table-sm align-middle mb-0">
                                    ...
                                    @foreach($group->directions as $direction)
                                        @php($directionDeliverables = $group->deliverables->where('organizational_unit_id',$direction->id))
                                        @php($directionEvidences = $directionDeliverables->flatMap->evidences)
                                        <tr>
                                            <td><i class="bi bi-diagram-3 me-1"></i>{{ $direction->name }}</td>
                                            <td>{{ $directionEvidences->count() }}</td>
                                            <td>{{ $directionEvidences->sum(fn($e) => $e->files->count()) }}</td>
                                            <td>{{ $directionDeliverables->filter(fn($d) => in_array($d->status instanceof \BackedEnum ? $d->status->value : $d->status,['VALIDADO','CERRADO'],true))->count() }}/{{ $directionDeliverables->count() }} validados</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
        ...
        <div class="siget-file-section border-top">
            ...
            @forelse($loads as $load)
                <div class="card border mb-3"><div class="card-header"><strong>{{ $load->title }}</strong> · {{ $load->agency?->name }} · {{ $load->period_label }}</div><div class="card-body"><div class="row g-3">
                @foreach($load->deliverables as $deliverable)
                    @php($evidence = $deliverable->evidences->sortByDesc('id')->first())
                    <div class="col-xl-6"><div class="border rounded p-3 h-100"><span class="badge text-bg-light">{{ $deliverable->organizationalUnit?->name }}</span><h3 class="h6 mt-2">{{ $deliverable->templateRequirement?->name }}</h3>
                        @if($evidence)<div class="small mb-2"><strong>{{ $evidence->title }}</strong> · {{ $evidence->files->count() }} archivo(s)</div>@endif
                        <form action="{{ route('evidences.store') }}" method="POST" enctype="multipart/form-data">@csrf<input type="hidden" name="deliverable_id" value="{{ $deliverable->id }}"><div class="input-group"><input name="title" class="form-control" placeholder="Título de la evidencia" value="{{ $evidence?->title }}"><input name="file" type="file" class="form-control" required><button class="btn btn-primary">Adjuntar</button></div></form>
                    </div></div>
                @endforeach
                </div></div></div>
            @empty<div class="text-secondary">No hay cargas disponibles.</div>@endforelse
        </div>
        @endif
        ...
        <div class="siget-file-section border-top">
            ...
            <div class="siget-file-grid">
                @forelse($recentFiles as $f)
                    <a href="{{ route('evidence-files.download',$f) }}" class="siget-file-card"><i class="bi bi-file-earmark"></i><span><strong>{{ $f->original_name }}</strong><small>{{ $f->evidence?->scheduledLoad?->agency?->name }} · {{ $f->evidence?->deliverable?->organizationalUnit?->name }}</small></span><small>{{ number_format($f->size_bytes/1024,1) }} KB</small></a>
