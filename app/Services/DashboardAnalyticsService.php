...
final class DashboardAnalyticsService
{
    ...
    public function forUser(User $user, array $filters = []): array
    {
        ...
        $monthlyTrend = (clone $base())->get(['effective_open_at','status'])->groupBy(fn ($load) => $load->effective_open_at->format('Y-m'))->groupBy(fn ($loads) => $loads->first()->effective_open_at->format('Y-m'))->sortKeys()->take(-12)->map(function ($loads, $period) {
        ...
        $unitPerformance = $deliverableQuery->join('organizational_units','organizational_units.id','=','scheduled_load_deliverables.organizational_unit_id')->whereIn('scheduled_load_deliverables.scheduled_load_id',$accessibleLoadIds)->select('organizational_units.name')->selectRaw('COUNT(*) AS total')->selectRaw("SUM(CASE WHEN scheduled_load_deliverables.status IN ('VALIDADO','CERRADO') THEN 1 ELSE 0 END) AS validated")->groupBy('organizational_units.id','organizational_units.name')->orderBy('organizational_units.name')->get()->map(fn ($row)=>['unit'=>$row->name,'total'=>(int)$row->total,'validated'=>(int)$row->validated,'percentage'=>(int)$row->total===0?0:round(100*(int)$row->validated/(int)$row->total,2)])->all();
        ...
        $directionPerformance = (clone $base())->join('organizational_units','organizational_units.id','=','scheduled_loads.organizational_unit_id')->select('organizational_units.id','organizational_units.name')->selectRaw('COUNT(*) AS total')->selectRaw("SUM(CASE WHEN scheduled_loads.status='VALIDADO_Y_CERRADO' THEN 1 ELSE 0 END) AS closed")->selectRaw("SUM(CASE WHEN scheduled_loads.status='VENCIDA' THEN 1 ELSE 0 END) AS overdue")->groupBy('organizational_units.id','organizational_units.name')->orderByDesc('overdue')->limit(30)->get()->map(fn($r)=>['unit'=>$r->name,'total'=>(int)$r->total,'closed'=>(int)$r->closed,'overdue'=>(int)$r->overdue,'percentage'=>(int)$r->total===0?0:round(100*(int)$r->closed/(int)$r->total,1)])->all();
        $riskItems = (clone $base())->with(['agency','organizationalUnit'])->whereIn('status',['VENCIDA','OBSERVADA','EN_REVISION_INSTITUCIONAL','PENDIENTE_DOCUMENTO_FIRMADO'])->orderByRaw("CASE status WHEN 'VENCIDA' THEN 1 WHEN 'OBSERVADA' THEN 2 ELSE 3 END")->orderBy('effective_close_at')->limit(15)->get();
    ...
    private function filteredBase(User $user, array $filters): Builder
    {
        ...
        if (!empty($filters['organizational_unit_id'])) $query->where('organizational_unit_id',(int)$filters['organizational_unit_id']);
