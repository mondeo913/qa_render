...
final class DashboardAnalyticsService
{
    ...
    public function forUser(User $user, array $filters = []): array
    {
        ...
        $directionPerformance = (clone $base())->join('scheduled_load_deliverables','scheduled_load_deliverables.scheduled_load_id','=','scheduled_loads.id')->join('organizational_units','organizational_units.id','=','scheduled_load_deliverables.organizational_unit_id')->select('organizational_units.id','organizational_units.name')->selectRaw('COUNT(DISTINCT scheduled_loads.id) AS total')->selectRaw("COUNT(DISTINCT CASE WHEN scheduled_loads.status='VALIDADO_Y_CERRADO' THEN scheduled_loads.id END) AS closed")->selectRaw("COUNT(DISTINCT CASE WHEN scheduled_loads.status='VENCIDA' THEN scheduled_loads.id END) AS overdue")->groupBy('organizational_units.id','organizational_units.name')->orderByDesc('overdue')->limit(30)->get()->map(fn($r)=>['unit'=>$r->name,'total'=>(int)$r->total,'closed'=>(int)$r->closed,'overdue'=>(int)$r->overdue,'percentage'=>(int)$r->total===0?0:round(100*(int)$r->closed/(int)$r->total,1)])->all();
    ...
    private function filteredBase(User $user, array $filters): Builder
    {
        ...
        if (!empty($filters['organizational_unit_id'])) $query->whereIn('scheduled_loads.id', function ($q) use ($filters) { $q->select('scheduled_load_id')->from('scheduled_load_deliverables')->where('organizational_unit_id',(int)$filters['organizational_unit_id']); });
