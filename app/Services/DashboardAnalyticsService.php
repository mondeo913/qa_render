...
final class DashboardAnalyticsService
{
    ...
    public function forUser(User $user, array $filters = []): array
    {
        ...
        $isDirectionDirector = in_array($roleCode, [
            'DIRECTOR_TRANSMISION',
            'DIRECTOR_PROGRAMACION_CONTINUIDAD',
        ], true);

        if ($isDirectionDirector) {
            $statusDistributionQuery->whereIn('status', [
                'PROGRAMADA',
                'REPROGRAMADA',
                'VALIDADO_Y_CERRADO',
                'VENCIDA',
    ...
    private function filteredBase(User $user, array $filters): Builder
    {
        ...
        if (!empty($filters['organizational_unit_id'])) {
            ...
            if ($unitIds) $query->whereIn('scheduled_loads.id', function ($q) use ($unitIds) {
                $q->select('scheduled_load_id')->from('scheduled_load_deliverables')->whereIn('organizational_unit_id', $unitIds);
            });
        }
        if (!empty($filters['status'])) {
            $allowedDirectorStatuses = [
                'PROGRAMADA',
                'REPROGRAMADA',
                'VALIDADO_Y_CERRADO',
                'VENCIDA',
            ...
            if ($isDirectionDirector) {
                if (in_array($filters['status'], $allowedDirectorStatuses, true)) {
        ...
        if (!empty($filters['from'])) $query->whereDate('scheduled_loads.effective_open_at', '>=', $filters['from']);
        if (!empty($filters['to'])) $query->whereDate('scheduled_loads.effective_open_at', '<=', $filters['to']);
        return $query;
    }
}
